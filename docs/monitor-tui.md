...
| `cliente` | `User-Agent` crudo del request (`RequestRow::client`, truncado) | Correlaciona un salto de `tools`/`total` con QUIÉN lo originó: un harness que rompe su propio diferido de tools al pasar por un `ANTHROPIC_BASE_URL` no-first-party es la firma del impuesto que el proxy induce (ver `docs/optimizer-tool-search.md` §3) |
| `tsearch` | Carga diferida de tools del dialecto Responses/Codex (`RequestRow::tool_search`): `eager` (medido, sin diferido este turno), `lazy:N` (cargó N tools vía `tool_search`), o `-` (no aplica / proxy viejo) | El diferenciador eager-vs-lazy POR CLIENTE que `tools` (siempre eager en `tools[]`) no puede dar: en este dialecto lo diferido viaja en `input[]`, no en `tools[]` (ver `docs/telemetry-per-request.md` §4.3). Junto a `cliente`, dice si `pi`/`opencode` están difiriendo de verdad |
| `flat` | Honestidad del cubo `(native)` (`RequestRow::tools_flattened`): `yes` = el cliente NO usa el namespacing `mcp__` (`pi`/`opencode`), así que su `(native)` puede ocultar MCP aplanado; `no` = hay tools `mcp__`, el `(native)` es de fiar; `-` = no aplica / proxy viejo | Un `yes` avisa de que la columna `tools` de esa fila NO se puede desglosar por servidor MCP: es peso sin atribuir, no herramientas nativas (ver `docs/telemetry-per-request.md` §4.4). Es una advertencia estructural, jamás una atribución inventada |
