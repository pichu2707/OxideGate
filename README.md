...
| **Qué servidor MCP pagas y no usas** | `GET /mcp` cruza los bytes por servidor con las invocaciones reales y da un veredicto: `used`, `unused`, `insufficient_data` o `not_applicable`. Cada fila lleva la evidencia que lo sostiene —peticiones concluyentes y descartes por motivo— y el umbral viaja en la respuesta porque es un juicio, no una medida. Ataca la palanca más grande del catálogo (−55.098 B) sin tocar el cable. Ver [§4.16](docs/telemetry-per-request.md) | ✅ |
