...
| **Qué tools se INVOCARON de verdad** | Campos `tools_invoked` y `server_tools_invoked`: los nombres que el modelo llamó en la respuesta, crudos y con repeticiones. Es la contrapartida de `tool_names` (lo declarado), y cruzar ambos sobre el histórico es lo único que permite decir «pagas por este servidor MCP y no lo invocas» — la palanca más grande del catálogo. Solo Anthropic por ahora: vacío significa «no se reconoció ninguna», nunca «no invocó nada». Ver [§4.15](docs/telemetry-per-request.md) | ✅ |
