...
| **De QUÉ es el bloque de instrucciones** | `instructions.by_heading`: reparte los bytes del `CLAUDE.md` entre sus cabeceras. Medido sobre uno real de 33.460 B, **cuatro secciones son el 87%** —y las cuatro son protocolo de un flujo que viaja en cada petición, se use o no. Las filas suman el bloque EXACTO (preámbulo + desborde acotado), el nivel 3 no corta porque fragmentaría justo la fila que hay que ver, y **los nombres no viajan** salvo que los pidas: son texto de una persona, no una etiqueta que el cliente ya mandaba. Ver [§4.13.1](docs/telemetry-per-request.md) | ✅ |
...
| `OXIDEGATE_INSTRUCTIONS_HEADINGS` | `on` publica los NOMBRES de las cabeceras en el desglose de `instructions`. Los tamaños salen siempre; esto añade solo la etiqueta, que es texto de tu `CLAUDE.md` y acaba en `telemetry.jsonl` y en `GET /requests` | `off` (sin nombres) |
...
`PgUp`/`PgDn` recorrer la pantalla entera. Los paneles se pintan a tamaño
completo aunque no quepan todos a la vez: se recorren, no se aprietan.
